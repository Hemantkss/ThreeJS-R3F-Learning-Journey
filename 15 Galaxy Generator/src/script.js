...
const galaxyTexture = textureLoader.load("/textures/particles/2.png");
...
const generateGalaxy = () => {
  // Destroy old galaxy
  if (points !== null) {
    geometry.dispose(); // Remove from memory
    material.dispose(); // Remove from memory
  ...
  for (let i = 0; i < parameters.count * 3; i++) {
    ...
    // positions
    const radius = parameters.radius * Math.random(); // Random radius between 0 and 5
    const spinAngle = radius * parameters.spin;
    const branchAngle =
      ((i % parameters.branches) / parameters.branches) * Math.PI * 2; // Angle between 0 and 2PI
