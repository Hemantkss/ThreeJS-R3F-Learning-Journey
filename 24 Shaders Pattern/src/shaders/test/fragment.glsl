...
void main() {
    ...
    // Pattern 32
    // vec2 roatedUvX = rotate(vUv, PI * 0.25, vec2(0.5));

    // vec2 lightUvX = vec2(roatedUvX.x * 0.1 + 0.45, roatedUvX.y * 0.5 + 0.25);
    // float lightX = 0.015 / distance(lightUvX, vec2(0.5));

    // vec2 lightUvY = vec2(roatedUvX.y * 0.1 + 0.45, roatedUvX.x * 0.5 + 0.25);
    // float lightY = 0.015 / distance(lightUvY, vec2(0.5));
    ...
    // Clamp Color Strenght
    strenght = clamp(strenght, 0.0, 1.0);
    // Color Version
    vec3 blackColor = vec3(0.0);
    vec3 uColor = vec3(vUv, 0.5);
    vec3 mixedColor = mix(blackColor, uColor, strenght);
